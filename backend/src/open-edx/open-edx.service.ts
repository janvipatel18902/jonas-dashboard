...
export class OpenEdxService {
  ...
  private async callOpenEdxApi<T>(path: string): Promise<T> {
    ...
    if (!lmsUrl) {
      throw new InternalServerErrorException('OPEN_EDX_LMS_URL not configured');
    }
  ...
  async getCourses(): Promise<CoursesApiResponse> {
    return this.callOpenEdxApi<CoursesApiResponse>('/api/courses/v1/courses/');
  }

  async getCourseDetails(courseId: string): Promise<CourseDetailResponse> {
    return this.callOpenEdxApi<CourseDetailResponse>(
      `/api/courseware/course/${courseId}`,
    );
  }

  async getCourseGradebook(courseId: string): Promise<GradebookResponse> {
    return this.callOpenEdxApi<GradebookResponse>(
      `/api/grades/v1/gradebook/${courseId}/`,
    );
  }

  async getCourseGrades(courseId: string): Promise<CourseGradesResponse> {
